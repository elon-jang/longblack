...
| 문제 | 상세 | 영향 |
|------|------|------|
| 한글 임베딩 한계 | all-MiniLM-L6-v2는 영어 중심 모델 | 한글 시맨틱 검색 정확도 낮음 |
| 특수 쿼리 처리 | "9.81파크" 같은 숫자+한글 조합 | FTS5 토큰화 실패 |
| 동의어 미지원 | "AI" vs "인공지능" 매칭 불가 | 관련 문서 누락 |
| 점수 통합 부재 | 제목/FTS/시맨틱 점수 단순 병합 | 최적 랭킹 아님 |

#### 개선 방향
1. **한글 임베딩 강화**: 다국어 모델 또는 한글 특화 모델 검토
2. **쿼리 전처리**: 숫자/특수문자 정규화, 형태소 분석
3. **하이브리드 스코어링**: RRF(Reciprocal Rank Fusion) 적용
4. **키워드 확장**: 동의어 사전, 관련어 확장
